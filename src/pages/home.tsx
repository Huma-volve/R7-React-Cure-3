...
const Home: React.FC = () => {
    ...
    const fetchFaqs = async () => {
      try {
        const res = await axios.get(
          "https://round7-cure.huma-volve.com/api/faqs"
        );
        const apiFaqs = res.data.data;

        const englishFaqs = apiFaqs.map((item: any) => ({
          q: item.translations?.en?.question ?? item.question,
          a: item.translations?.en?.answer ?? item.answer,
        }));
  ...
  return (
    <div className="min-h-screen px-2 md:px-12 bg-white text-gray-800">
      ...
      <section  id="FAQ"  className="scroll-mt-32 py-12 md:px-4">
        ...
        >
          ...
          <div className="w-full lg:w-1/3 flex justify-center">
            <img
              src="down.png"
              alt="app"
              className="w-[260px] sm:w-[330px] md:w-[380px] lg:w-full"
            />
          </div>
        </div>
      </section>
