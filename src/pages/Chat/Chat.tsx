...
//   // Dummy current user (replace with your auth)
//   // const currentUser = { id: "1", name: "You", avatar: "https://i.pravatar.cc/150?img=1" };
...
//     try {
//       setSelectedConv(conv);
//       const response = await axios.get(`${API_BASE}chats/${conv.id}/messages`);
...
//     const recipient = selectedConv.participants.find((p) => p !== currentUser.id)!;
...
//         `${API_BASE}chats/${selectedConv.id}/messages`,
//         formData,
//         { headers: { "Content-Type": "multipart/form-data" } }
//       );

//       const serverMsg = response.data.data || response.data;
//       setMessages((prev) =>
//         prev.map((m) => (m.id === optimistic.id ? serverMsg : m))
//       );
//     } catch (error) {
...
//     return { id: otherId, name: `User ${otherId}`, avatar: `https://i.pravatar.cc/150?img=${otherId}` };
//   }, [selectedConv]);
...
//                 onClick={() => selectConversation(c)}
//                 className={`w-full flex items-center p-3 gap-3 text-left transition ${
//                   selectedConv?.id === c.id ? "bg-gray-100" : "hover:bg-gray-50"
//                 }`}
//               >
//                 <img src={`https://i.pravatar.cc/150?img=${c.id}`} alt={c.name} className="w-10 h-10 rounded-full" />
//                 <div className="flex-1">
//                   <div className="font-medium">{c.name}</div>
//                   <div className="text-sm text-gray-500 truncate">{c.lastMessage}</div>
//                 </div>
//                 {c.unread > 0 && (
//                   <span className="ml-2 text-xs bg-blue-500 text-white px-2 py-0.5 rounded-full">{c.unread}</span>
//                 )}
//               </button>
//             ))}
//           </div>
...
//           {/* Header */}
//           <div className="p-4 border-b border-gray-300 flex items-center gap-4 bg-gray-50">
//             <button onClick={() => setMobileView("list")} className="md:hidden mr-2 text-gray-700 hover:text-gray-900">
//               <ArrowLeft className="w-5 h-5" />
//             </button>
//             {otherUser ? (
//               <>
//                 <img src={otherUser.avatar} alt={otherUser.name} className="w-11 h-11 rounded-full" />
//                 <div>
//                   <div className="font-semibold">{otherUser.name}</div>
//                   <div className="text-xs text-green-600">online</div>
//                 </div>
...
//                 <div
//                   key={m.id}
//                   className={`flex ${m.from === currentUser.id ? "justify-end" : "justify-start"} mb-3`}
//                 >
//                   <div
...
//                       <div className="mt-2">
//                         {m.file.type.startsWith("image/") ? (
//                           <img src={m.file.url} alt={m.file.name} className="rounded-lg max-w-[200px]" />
//                         ) : m.file.type.startsWith("audio/") ? (
//                           <audio src={m.file.url} controls />
//                         ) : (
//                           <a href={m.file.url} download={m.file.name} className="text-sm underline">
//                             {m.file.name}
//                           </a>
//                         )}
//                       </div>
